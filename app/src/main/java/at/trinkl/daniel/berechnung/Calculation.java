...
public class Calculation {
    ...
    private final double faValue = 0.97;
    ...
    public double calculate(double input, double fa) {
        ...
        if (dole > azr && fa > 0){
            double doleFa = dole + (faValue * fa);
            doleFa = Math.round(100.0 * doleFa) / 100.0;
            dole = doleFa;
        }

        if (dole < azr && fa == 0) {
            double dole60 = dailynet * 0.60;
            dole60 = Math.round(100.0 * dole60) / 100.0;
            dole = dole60;
        } else if (dole < azr && fa > 0) {
            double dole80 = dailynet * 0.80;
            dole80 = Math.round(100.0 * dole80) / 100.0;
            if (dole80 > azr) {
                double dole80HigherAzr = azr + (faValue * fa);
                dole80HigherAzr = Math.round(100.0 * dole80HigherAzr) / 100.0;
                dole = dole80HigherAzr;
            } else {
                dole = dole80 + fa;
            }
        }
